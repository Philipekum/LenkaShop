...
class YooKassaPaymentService:
    ...
    def create_payment(order, total_price, request):
        try:
            ...
            builder.set_amount({"value": f"{total_price:.2f}", "currency": Currency.RUB}) \
                .set_confirmation({
            ...
            logger.info(f"Created payment {payment_response.id} for order {order.order_id}")
            ...
            logger.error(f"Error creating payment for order {order.order_id}: {str(e)}")
            raise
